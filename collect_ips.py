...
# 获取IP的国家信息（改进版本）
def get_ip_country(ip: str, max_retries: int = 2) -> str:
    """获取IP地址对应的国家，使用多个备用API"""
    apis = [
        # API 1: ipapi.co (JSON格式)
        {
        ...
        # API 4: 国内API
        {
            'url': f'https://ipapi.ipapi.com/api/{ip}',
            'field': 'country_name'
        }
    ]
        ...
        for retry in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if country and country != 'Unknown' and country != '未知':
                        # 简化国家名称（如果是英文，可以保持原样）
    ...
    # 如果所有API都失败，根据IP段判断（Cloudflare IP段）
